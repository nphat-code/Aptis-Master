...
import { buildSpeakingPart3GeminiPrompt } from '@/utils/geminiPrompts';
...
interface SpeakingPart3ViewProps {
...
export default function SpeakingPart3View({
  data,
  currentSubIndex,
  onNextQuestion,
  onPrevQuestion,
}: SpeakingPart3ViewProps) {
  ...
  const handleCopy = () => {
    if (!currentQ) return;
    const promptText = buildSpeakingPart3GeminiPrompt(currentQ.num, currentQ.text);
    ...
    <div className="max-w-4xl mx-auto space-y-6 animate-slide-question">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-slate-500 font-medium">Part 3 – Compare pictures</span>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="bg-white p-3 rounded-2xl border border-slate-200/90 shadow-2xs space-y-2">
          ...
          <div className="relative w-full aspect-4/3 rounded-xl overflow-hidden bg-slate-100 border border-slate-100">
            {data.imageUrl1 ? (
              <Image
                src={data.imageUrl1}
                alt="Picture 1"
        ...
        {showSample && currentQ.sampleAnswer && (
          <div className="p-4 bg-[#ecfdf5] border border-emerald-300/80 rounded-2xl space-y-2 animate-fast-fade text-[14px]">
            ...
            <div
              ...
              dangerouslySetInnerHTML={{ __html: currentQ.sampleAnswer }}
            />
          </div>
        )}
      </div>
