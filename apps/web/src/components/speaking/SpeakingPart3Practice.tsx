...
export default function SpeakingPart3Practice({
  testIndex = 0,
  onExit,
}: SpeakingPart3PracticeProps) {
  const isAllPractice = testIndex === -1;
  const rawSpeaking = (scrapedData as any)?.speaking || {};
  const rawPart3: any[] = rawSpeaking.part3_practice || [];
  const totalSets = rawPart3.length || 25;

  const safeTestIndex = isAllPractice ? 0 : (((testIndex % totalSets) + totalSets) % totalSets);

  const allTestData: SpeakingPart3Data[] = useMemo(() => {
    return rawPart3.map((item, idx) => ({
      id: `p3_set_${idx + 1}`,
      imageUrl1: item.urlpic1 || '',
      imageUrl2: item.urlpic2 || '',
      questions: [
        {
          num: 1,
          text: item.question1 || 'Describe and compare the pictures?',
      ...
      renderDetailedAnswers={() => {
        return (
          <div className="space-y-6">
            <DetailedAnswersCard
              title="Tổng hợp câu hỏi & gợi ý bài mẫu Part 3"
              subtitle="So sánh 2 bức tranh và trả lời 2 câu hỏi mở rộng. Thời gian chuẩn: 45 giây cho mỗi câu."
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                ...
                {currentData.imageUrl2 && (
                  <div className="relative w-full aspect-4/3 rounded-2xl overflow-hidden bg-slate-100 border border-slate-200">
                    <Image
                      src={currentData.imageUrl2}
                      alt="Part 3 Picture 2"
                      fill
                      className="object-cover"
                      sizes="(max-width: 768px) 100vw, 350px"
                    />
                    ...
                    {q.sampleAnswer && (
                      <div className="p-4 bg-[#ecfdf5] border border-emerald-200/80 rounded-xl space-y-1.5 text-[14px]">
                        ...
                        <div
                          ...
                          dangerouslySetInnerHTML={{ __html: q.sampleAnswer }}
                        />
                      </div>
                    )}
                  </div>
